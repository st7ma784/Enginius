...
class PosteriorVisualizer:
    ...
    def plot_posterior_distributions(self, samples: Dict, 
        ...
        for i, (param_name, param_samples) in enumerate(scalar_samples.items()):
            if i < len(axes):
                ...
                # Histogram with KDE
                ax.hist(param_samples, bins=50, alpha=0.6, density=True, 
                ...
                ax.axvline(q025, color='orange', linestyle='-', alpha=0.7,
    ...
    def plot_trace_plots(self, samples: Dict, 
                        param_names: List[str] = None,
                        save_path: str = None) -> plt.Figure:
        """
    ...
    def plot_correlation_matrix(self, samples: Dict, 
                              param_names: List[str] = None,
                              save_path: str = None) -> plt.Figure:
        """
    ...
    def create_interactive_posterior_plot(self, samples: Dict, 
                                        param_names: List[str] = None) -> go.Figure:
        """
        ...
        for i, (param_name, param_samples) in enumerate(scalar_samples.items()):
            ...
            # Histogram
            fig.add_trace(
                go.Histogram(x=param_samples, nbinsx=50, opacity=0.7,
    ...
    def plot_prediction_intervals(self, X_test: np.ndarray, y_true: np.ndarray,
        ...
        axes[0, 0].errorbar(y_true, y_pred, yerr=y_std, fmt='none', 
                           alpha=0.3, capsize=2)
        ...
        axes[0, 1].scatter(x_range, y_true_sorted, c='red', s=10, 
        ...
        axes[1, 1].axvline(np.mean(y_std), color='red', linestyle='--',
    ...
    def plot_feature_importance(self, samples: Dict, feature_names: List[str],
                              save_path: str = None) -> plt.Figure:
        """
        ...
        for i, (name, samples_array) in enumerate(coef_samples.items()):
            ...
            axes[1].hist(samples_array, bins=30, alpha=0.6, 
                        label=clean_name, density=True)
