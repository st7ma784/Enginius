...
class WandBLogger:
    def __init__(self, project_name: str = "car-collection-wait-times", 
    ...
    def _create_data_visualizations(self, df: pd.DataFrame):
        ...
        axes[1, 1].bar(['Non-EV', 'EV'], ev_comparison.values, 
    ...
    def log_posterior_distributions(self, samples: Dict, param_names: List[str] = None):
        ...
        for i, (param_name, param_samples) in enumerate(scalar_params.items()):
            if i < len(axes):
                ...
                axes[i].axvline(q025, color='orange', linestyle=':', 
    ...
    def log_prediction_results(self, y_true: np.ndarray, y_pred: np.ndarray, 
                              y_std: np.ndarray, feature_names: List[str] = None):
