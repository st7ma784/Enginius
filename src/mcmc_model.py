...
class BayesianWaitTimeModel:
    ...
    def model(self, X: torch.Tensor, y: Optional[torch.Tensor] = None):
        ...
        with pyro.plate("locations", self.n_locations):
            location_effects = pyro.sample(
                "location_effects", dist.Normal(location_mean, location_std)
            )
        ...
        with pyro.plate("drivers", self.n_drivers):
            driver_effects = pyro.sample(
                "driver_effects", dist.Normal(driver_mean, driver_std)
            )
        ...
        checklist_coef = pyro.sample(
            "checklist_coef", dist.Normal(2.0, 1.0)
        )  # ~2 min per item
        ...
        charge_coef = pyro.sample(
            "charge_coef", dist.Normal(45.0, 15.0)
        )  # 45 min for charging
        lunch_coef = pyro.sample(
            "lunch_coef", dist.Normal(15.0, 5.0)
        )  # 15 min for lunch
    ...
    def fit(
        ...
        print(
            f"Using {num_chains} chains with max {self.max_cpu_threads} CPU threads on {self.device}"
        )
        ...
        try:
            ...
            try:
                ...
                if "signal only works in main thread" in str(e):
                    ...
                    print(
                        "Warning: Multi-threading signal issue detected. Falling back to single chain..."
                    )
                    mcmc = MCMC(
                        nuts_kernel,
                        num_samples=num_samples,
    ...
    def fit_streamlit_safe(
        ...
        try:
            ...
            raise RuntimeError(
                f"MCMC training failed in Streamlit environment: {str(e)}"
            )
    ...
    def predict(
        self, X_new: np.ndarray, num_samples: int = 500
        ...
        predictive = pyro.infer.Predictive(
            self.model, posterior_samples=self.samples, num_samples=num_samples
        )
