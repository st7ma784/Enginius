...
class BookingEnvironment:
    ...
    def __init__(
        self, n_locations: int = 6, n_drivers: int = 20, max_episodes: int = 100
    ):
    ...
    def reset(self) -> BookingState:
        ...
        self.available_drivers = np.random.choice(
            self.n_drivers, n_available, replace=False
        )
        ...
        self.public_transport_cost = self.public_transport_base_cost * (
            1 + np.random.normal(0, 0.2)
        )
        self.public_transport_time = self.public_transport_base_time * (
    ...
    def step(self, action: Action) -> Tuple[BookingState, float, bool, Dict]:
        ...
        if action.action_type == "book_driver":
            if action.driver_id in self.available_drivers:
                ...
                actual_wait_time = self.estimated_wait_times[driver_idx] * (
                    1 + np.random.normal(0, 0.2)
                )
            ...
            reward = (
                self.booking_reward_base
                - self.public_transport_cost
                - self.public_transport_time * 0.2
            )
            ...
            if np.random.random() < 0.3:  # 30% chance of changes
                ...
                if len(self.available_drivers) > 1:
                    ...
                    self.available_drivers = [
                        d for d in self.available_drivers if d not in to_remove
                    ]
                    ...
                    self.estimated_wait_times = [
                        self.estimated_wait_times[i] for i in keep_indices
                    ]

                # New drivers might become available
                unavailable_drivers = [
                    d for d in range(self.n_drivers) if d not in self.available_drivers
                ]
                if unavailable_drivers and np.random.random() < 0.2:
                    new_driver = np.random.choice(unavailable_drivers)
                    self.available_drivers = np.append(
                        self.available_drivers, new_driver
                    )
                    new_wait_time = np.random.exponential(
                        18
                    )  # Slightly better wait time
            ...
            if self.booking_deadline <= 0:
                ...
                reward += (
                    self.booking_reward_base - self.public_transport_cost * 1.5 - 20
                )  # Deadline penalty
...
class DQNAgent:
    ...
    def act(self, state: BookingState) -> Action:
        ...
        state_vector = (
            torch.FloatTensor(self._state_to_vector(state)).unsqueeze(0).to(self.device)
        )
    ...
    def replay(self, batch_size: int = 32):
        ...
        states = torch.FloatTensor([self._state_to_vector(e.state) for e in batch]).to(
            self.device
        )
        rewards = torch.FloatTensor([e.reward for e in batch]).to(self.device)
        next_states = torch.FloatTensor(
            [self._state_to_vector(e.next_state) for e in batch]
        ).to(self.device)
        ...
        # Compute loss
        predicted_q_values = current_q_values.gather(
...
class RLBenchmark:
    ...
    def train_rl_agent(self, episodes: int = 1000) -> Tuple[DQNAgent, List[float]]:
        ...
        for episode in range(episodes):
            ...
            if episode % 100 == 0:
                ...
                print(
                    f"Episode {episode}, Average Reward: {avg_reward:.2f}, Epsilon: {agent.epsilon:.3f}"
                )
    ...
    def _mcmc_decision(self, state: BookingState) -> Action:
        ...
        for i, driver_id in enumerate(state.driver_available):
            ...
            try:
                ...
                driver_predictions.append(
                    (driver_id, state.estimated_wait_times[i], 5.0)
                )

        # Decision logic based on predictions and uncertainty
        best_driver = min(
            driver_predictions, key=lambda x: x[1]
        )  # Minimum predicted wait
        best_wait_time = best_driver[1]

        # Compare with public transport
        pt_effective_time = (
            state.public_transport_time + state.public_transport_cost * 0.5
        )  # Cost penalty
        ...
        if best_wait_time < pt_effective_time * 0.7:  # Driver is clearly better
            ...
            best_wait_time > pt_effective_time * 1.3
        ):  # Public transport is clearly better
            return Action("public_transport")
        else:
    ...
    def compare_methods(
        self, rl_episodes: int = 1000, eval_episodes: int = 200
    ) -> Dict:
    ...
    def plot_comparison(self, results: Dict, save_path: str = None):
        ...
        axes[0, 1].hist(
            results["rl_eval_rewards"], bins=30, alpha=0.7, label="RL Agent"
        )
        axes[0, 1].hist(
            results["mcmc_rewards"], bins=30, alpha=0.7, label="MCMC Policy"
        )
        ...
        # Box plot comparison
        axes[1, 0].boxplot(
            [results["rl_eval_rewards"], results["mcmc_rewards"]],
        ...
        window = 50
        rl_running_avg = (
            pd.Series(results["rl_eval_rewards"]).rolling(window=window).mean()
        )
        mcmc_running_avg = (
            pd.Series(results["mcmc_rewards"]).rolling(window=window).mean()
        )

        axes[1, 1].plot(
            rl_running_avg, label=f'RL Agent (avg: {results["rl_mean"]:.2f})'
        )
        axes[1, 1].plot(
            mcmc_running_avg, label=f'MCMC Policy (avg: {results["mcmc_mean"]:.2f})'
        )
