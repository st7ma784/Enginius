...
def main():
    parser = argparse.ArgumentParser(description='Train Car Collection Wait Time Prediction Model')
    parser.add_argument('--n_samples', type=int, default=2000, help='Number of training samples')
    parser.add_argument('--mcmc_samples', type=int, default=1000, help='Number of MCMC samples')
    parser.add_argument('--warmup_steps', type=int, default=500, help='MCMC warmup steps')
    parser.add_argument('--num_chains', type=int, default=None, help='Number of MCMC chains (auto-capped at 8)')
    parser.add_argument('--use_wandb', action='store_true', help='Use Weights & Biases logging')
    parser.add_argument('--wandb_project', type=str, default='car-collection-wait-times', help='WandB project name')
    ...
    if args.use_wandb:
        ...
        run = wandb_logger.start_run(run_name=f"mcmc_training_{args.mcmc_samples}samples")
    ...
    fig_posterior = visualizer.plot_posterior_distributions(
        results['samples'],
        save_path=os.path.join(args.output_dir, 'visualizations', 'posterior_distributions.png')
    )
    ...
    fig_corr = visualizer.plot_correlation_matrix(
        results['samples'],
        save_path=os.path.join(args.output_dir, 'visualizations', 'correlation_matrix.png')
    )
    ...
    fig_pred = visualizer.plot_prediction_intervals(
        X_test, y_test, y_pred, y_std,
        save_path=os.path.join(args.output_dir, 'visualizations', 'prediction_analysis.png')
    )
